...
@not_found:
        mvax    node_ptr, match_ptr     ; Copy into match_ptr

; Fall through

; Outputs a name.
; Does NOT add whitespace; callers must add (or not).
; match_ptr = pointer to the start of the name (note name_length is not required)
...
list_directive:
        ...
        ldphaa  node_ptr                ; Save existing value of node_ptr
...
@pop_state:
        plstaa  node_ptr                ; Recover values previously saved on stack
        rts
