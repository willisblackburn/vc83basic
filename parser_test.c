...
static void test_parse_element(void) {
    ...
    call_parse_element("ON X/2 GOTO 10,20,30", 0, 2, line_data_7, sizeof line_data_7, __LINE__);

    // Test that the parser can differentiate between ON...GOTO and ON...GOSUB.

    call_parse_element("ON X GOSUB 10,20,30", 0, 2, line_data_8, sizeof line_data_8, __LINE__);
